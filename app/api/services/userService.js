...
class UserService {
    async createUser({firstName, lastName, email, password, role, storageId, subdomain, fingerprint}){
        const options = {};
        ...
        const user = await db().User.create({firstName, lastName, email, password: hashedPassword, role, options, storageId});
